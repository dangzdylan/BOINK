//
//  checklist.swift
//  boink
//
//  Created by Dylan Dang on 9/11/21.
//

import Foundation

/*
 **GOALS**\
 


 //change coin texture √
//monster speedup animation √
 /!! FIX SPEED SCALING !! √
 //replay button texture √
 /touch up coin texture √
 //death animation texture √
 //fix sound errors √
 //fix didbegincontact not calling/collision bug √
 //launch screen, touch up background √
 //LEADERBOARD √

 
 
 //object for skin √
 //save coin purse √
 //crate buy and opening screen (W/ RARITY ANIMATION), random alg
 
 //redo leaderboard, shop and inventory button √
 /*inventory screen √
   - all skins with scroll ui
   - shows which one is equipped
  
  */
 //coin adding animation √
 
 //more coming soon box √
 //backgrounds
 //skins
 //crate texture

 

 
 
 
 
 
 //monster idle animation√
 //FIX SCALING AND MONSTER SPEED WALL SLIDE PROBLEM √
 -FONT FONT FONT FONT √
 -save coins as currency √
 -FIX BLACK BARS √
 - fix player texture rotation bug √
 - save highscore √
 - fix up start and end screen √
 - fix random player impulse at start √
 - make monster spawn into playfield and bounce around at constant speed √
 - fix coin moving random algorithm √
 - figure out how DIDBEGINCONTACT and contactbitmasking works √
 - organize objects; put in different files and make classes √
 
 
 */



